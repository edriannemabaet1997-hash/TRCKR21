...
def process_run_prob(rate, pa, pitcher_era, bullpen_era, park, platoon, iso, team_obp) -> float:
    ...
    return compute_event_probability(rate * team_factor, pa, starter_mult, bullpen_mult, park, platoon, iso)


def process_hr_prob(rate, pa, pitcher_era, bullpen_era, park, platoon, iso) -> float:
    starter_mult = 1.0 + ((4.20 - pitcher_era) / 10.0) * 1.5
    bullpen_mult = 1.0 + ((4.20 - bullpen_era) / 10.0) * 1.5
    return compute_event_probability(rate, pa, starter_mult, bullpen_mult, park * 1.1, platoon, iso * 1.3)


def process_rbi_prob(rate, pa, pitcher_era, bullpen_era, park, platoon, iso, team_obp) -> float:
    ...
    return compute_event_probability(rate * lineup_factor, pa, starter_mult, bullpen_mult, park, platoon, iso * 1.1)


def calibrate_with_statcast_full(
    original_prob: float, xwoba: float, xba: float, barrel: float, hard_hit: float, whiff: float
) -> dict:
