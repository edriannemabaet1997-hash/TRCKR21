...
-----------------------
  1. The schedule call now hydrates `probablePitcher` explicitly. The MLB
     Stats API does NOT include probable-pitcher data on the bare
     `/schedule` endpoint — it's an opt-in hydration. Without it, every
     game['teams'][side] dict is missing the 'probablePitcher' key, so
     the old code silently queued zero pitcher jobs, every slate produced
     an empty `pitchers` array, and the frontend correctly (but
     confusingly) disabled the Pitcher Props tab because it had nothing
     to show. This was the root cause of "I can't click Pitcher Props."
  2. Every pitcher and team record now carries today's actual opponent
     (`opponent`, `isHome`, and a human-readable `matchup` string like
     "vs Cincinnati Reds" / "@ Yankee Stadium"'s team), and that matchup
     is folded into the `sub` line the frontend already renders in the
     sidebar and header — so team cards are no longer just "Team
     Matchup" with no indication of who they're playing today.
"""
...
def fetch_json(url: str) -> dict:
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            req = urllib.request.Request(url, headers={"User-Agent": "trckr21-quant-engine/2.1"})
...
def matchup_label(opponent_name: str, is_home: bool) -> str:
    """Human-readable label for *today's* game, e.g. 'vs Cincinnati Reds'
    or '@ New York Yankees'. Distinct from format_date_label, which labels
    a single row in the L10 history chart."""
...
def get_pitcher_data(pitcher_id: int, pitcher_name: str, team_name: str,
    ...
    try:
        ...
        for g in games:
            s = g["stat"]
            # MLB Stats API field is camelCase "strikeOuts" (capital O) —
            # "strikeouts" (all lowercase) does not exist on the stat
            # object and was silently defaulting every game to 0, which is
            # why every pitcher's K market showed a flat AVG 0.0.
...
def get_slate(date_str: str) -> list[dict]:
    # `hydrate=probablePitcher` is REQUIRED — without it the MLB Stats API
    # omits the 'probablePitcher' key entirely from every game['teams'][side]
    # dict, which silently zeroes out every pitcher job downstream.
...
def run(date_str: Optional[str] = None, output_path: str = "projections.json") -> None:
    ...
    for game in games:
        ...
        for side, opp_side in (("away", "home"), ("home", "away")):
            ...
            # NOTE: on doubleheader dates a team appears in two games; we
            # only keep the first game's opponent/matchup for that team's
            # card (a known, deliberate simplification — flag if you need
            # both games represented separately).
    ...
    if pitcher_count_expected == 0:
        log.warning("No probable pitchers were listed for any game on this slate "
                    ...
                    "MLB usually locks these in a few hours before first pitch). "
