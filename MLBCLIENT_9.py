...
class MLBClient:
    ...
    # NEW (2026-09-06, diagnostic pass) — Player Card scouting tool, replacing
    # the manual Statcast-calibration sliders. MLB's byDateRangeAdvanced +
    # sitCodes combo (season platoon split intersected with a date window) is
    # inconsistent across the free Stats API, so this derives the "last N
    # games, split by opposing starter's hand" reading directly and reliably:
    # pull the batter's last N game-log lines, then for each individual game
    # look up that game's actual opposing starter from the boxscore and
