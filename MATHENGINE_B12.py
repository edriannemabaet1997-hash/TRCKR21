...
# ---------------------------------------------------------------------------
# TASK 1 UI (2026-08-31) — weather_wind_context. Same directional math as
# weather_scoring_multiplier() above (kept in sync deliberately — this is a
# *display* helper, not a second model), converted into a short badge label,
# a "boost"/"penalty"/"neutral" tone the frontend maps to --green/--amber/
# neutral, and a full sentence for the hover tooltip. Lives here (not
# weather_client.py) because it's pure conversion with no I/O, same house
# convention as weather_scoring_multiplier.
# ---------------------------------------------------------------------------
WEATHER_CALM_WIND_MPH = 3.0
...
def _compass_label(deg: float | None) -> str | None:
    if deg is None:
        return None
...
def weather_wind_context(
    wind_speed_mph: float,
    wind_direction_deg: float | None = None,
    park_azimuth_deg: float | None = None,
) -> tuple[str, str, str]:
    """Human-readable wind context for the Moneylines weather badge.

    Returns (label, tone, detail):
      label  - short text next to the temperature, e.g. "Blowing Out 8mph",
               "Blowing In 5mph", "Crosswind 6mph", "Calm".
      tone   - "boost" | "penalty" | "neutral" — frontend maps these to
               --green / --amber / neutral badge styling respectively.
      detail - full sentence for the hover tooltip, e.g.
               "Wind: 8mph SW, blowing toward center field".

    wind_direction_deg is the compass bearing the wind is blowing FROM
    (Open-Meteo convention, same as weather_scoring_multiplier's input);
    park_azimuth_deg is the home-plate -> center-field bearing the park
    faces TOWARD (mlb_client.venue()'s location.azimuthAngle).
    """
    speed = round(safe_float(wind_speed_mph, 0.0))
    compass = _compass_label(wind_direction_deg)

    if not wind_speed_mph or wind_speed_mph < WEATHER_CALM_WIND_MPH:
        detail = f"Wind: calm{f' ({compass})' if compass else ''}"
        return "Calm", "neutral", detail

    if park_azimuth_deg is None or wind_direction_deg is None:
        label = f"Wind {compass} {speed}mph" if compass else f"Wind {speed}mph"
        detail = f"Wind: {speed}mph{f' {compass}' if compass else ''} — park orientation unknown"
        return label, "neutral", detail

    blowing_toward_deg = (wind_direction_deg + 180.0) % 360.0
    angle_diff = abs((blowing_toward_deg - park_azimuth_deg + 180.0) % 360.0 - 180.0)

    if angle_diff <= 45.0:
        descriptor, tone, toward = "Blowing Out", "boost", "blowing toward center field"
    elif angle_diff >= 135.0:
        descriptor, tone, toward = "Blowing In", "penalty", "blowing in from center field"
    else:
        descriptor, tone, toward = "Crosswind", "neutral", "blowing crosswind"

    label = f"{descriptor} {speed}mph"
    compass_part = f"{speed}mph {compass}" if compass else f"{speed}mph"
    detail = f"Wind: {compass_part}, {toward}"
